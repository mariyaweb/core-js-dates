...
function getTime(date) {
  ...
  const min = convertedDate.getMinutes().toString().padStart(2, '0');
  const sec = convertedDate.getSeconds().toString().padStart(2, '0');
  return `${h}:${min}:${sec}`;
}
...
function formatDate(date) {
  ...
  const fullYear = currDate.getUTCFullYear();
  const hours = currDate.toLocaleTimeString('en-US', {
    hour: 'numeric',
    minute: 'numeric',
    second: 'numeric',
    hour12: true,
  ...
  return `${month}/${day}/${fullYear}, ${hours}`;
}
...
function getNextFridayThe13th(date) {
  ...
  if (day === 13) {
    the13th.setUTCMonth(the13th.getUTCMonth() + 1);
  }

  while (the13th.getDay() !== 5) {
    the13th.setUTCMonth(the13th.getUTCMonth() + 1);
  }
